...
/**
 * Created by hongliJiang on 2019/8/21 15:35
 * 描述：行程日期选择 初始结束时间选择
 */
public class CalendarSelectView extends FrameLayout {
    private static final String TAG = "日历";
    ...
    public CalendarSelectView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public CalendarSelectView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    public void setShowDate(String startDate, String endDate) {
        adapter.data.addAll(days(startDate, endDate));
...
//        recyclerView.addItemDecoration(myItemD);
        ...
        adapter.setOnRecyclerviewItemClick((v, position) -> onClick(adapter.data.get(position), position));
    }

    private void onClick(DateBean dateBean, int position) {
        ...
        if (startDate == null) {
            ...
            if (onDateSelected != null) {
                onDateSelected.selected(simpleDateFormat.format(startDate.getDate()), null);
            }
            ...
            if (startDate == dateBean) {
                ...
                //选中结束日期
                if ((dateBean.getDate().getTime() - startDate.getDate().getTime()) / (60 * 60 * 1000 * 24) > 30) {
                    showInfoWindow();
                    return;
                }
                endDate = dateBean;
    ...
    //设置默认选中的日期
    public void setSelectedRande(String defaultStartDate, String defaultEndDate) {
        if (adapter == null) return;
        try {
            Date starDate = simpleDateFormat.parse(defaultStartDate);
            Date endDateDefalut = simpleDateFormat.parse(defaultEndDate);
            ArrayList<DateBean> data = adapter.data;
            for (int i = 0; i < data.size(); i++) {
                DateBean dateBean = data.get(i);
                if (dateBean == null||dateBean.getDate()==null) continue;
                if (dateBean.getDate().compareTo(starDate) == 0) {
                    startDate = dateBean;
                    startDate.setItemState(DateBean.ITEM_STATE_BEGIN_DATE);
                }
                if (dateBean.getDate().compareTo(endDateDefalut) == 0) {
                    endDate = dateBean;
                    endDate.setItemState(DateBean.ITEM_STATE_END_DATE);
                }
            }
    ...
    //
    private void changeStarPositionBg() {
        if (endDate != null && startDate != null) {
            int start = adapter.data.indexOf(startDate);
            DateBean dateBean = adapter.data.get(start);
            dateBean.setItemState(DateBean.ITEM_STATE_STAR_CHANGE);
        }
    }
    ...
    public class CalendarAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
            if (viewHolder instanceof CalendarAdapter.MonthViewHolder) {
                ...
                DateBean dateBean = data.get(i);
                //是否可选中
                boolean canClickDate = data.get(i).isCanClickDate();
                int weekStr = getWeekStr(dateBean.getDate());


                if (canClickDate) {
                    vh.itemView.setClickable(true);
                    //设置item状态
                    if (dateBean.getItemState() == DateBean.ITEM_STATE_BEGIN_DATE || dateBean.getItemState() == DateBean.ITEM_STATE_END_DATE || dateBean.getItemState() == DateBean.ITEM_STATE_STAR_CHANGE) {
                        //开始日期或结束日期
                        vh.tv_day.setVisibility(GONE);
                        ...
                        if (dateBean.getItemState() == DateBean.ITEM_STATE_END_DATE) {
                            vh.itemView.setBackgroundResource(R.drawable.layer_list_calendar_day_left);
                            vh.tv_check_in_check_out.setText("返回");
                        } else if (dateBean.getItemState() == DateBean.ITEM_STATE_STAR_CHANGE) {//开始背景状态改变了
                            vh.itemView.setBackgroundResource(R.drawable.layer_list_calendar_day_right);
                            vh.tv_check_in_check_out.setText("出发");
                        } else {
                            vh.itemView.setBackgroundResource(R.drawable.shape_blue_radius_4);
                            vh.tv_check_in_check_out.setText("出发");
                        }
                    } else if (dateBean.getItemState() == DateBean.ITEM_STATE_SELECTED) {
                        //选中状态
                        vh.itemView.setBackgroundColor(Color.parseColor("#6649a6ff"));
                        vh.tv_day.setVisibility(VISIBLE);
                        if (weekStr == 1 || weekStr == 7) {//周六周天
                            vh.tv_day.setTextColor(Color.parseColor("#FDAD00"));
                        } else {
                            vh.tv_day.setTextColor(Color.parseColor("#484848"));
                        }
                    } else {
                        vh.tv_day.setVisibility(VISIBLE);
                        //正常状态
                    ...
                    vh.tv_day.setTextColor(Color.parseColor("#C1C1C1"));
                }
            }
        }
         ...
         * @return
         */
        private int getWeekStr(Date date) {
        ...
        public class DayViewHolder extends RecyclerView.ViewHolder {
            ...
            public DayViewHolder(@NonNull View itemView) {
                ...
                ViewGroup.LayoutParams layoutParams = tv_day.getLayoutParams();
                layoutParams.height = ScreenUtils.getScreenWidth(itemView.getContext()) / 7;
                tv_day.requestLayout();
            }
        }
...
//            Log.d(TAG, "endDateStr:" + endDateStr + "---------endDate:" + format.format(endDate));
            ...
            for (; calendar.getTimeInMillis() <= endDate.getTime(); ) {
                ...
                for (; monthCalendar.getTimeInMillis() <= endMonthDay.getTime(); ) {
                    ...
                    dateBean.setCanClickDate(canClickDateRange(startDate, endDate, monthCalendar.getTime()));
     ...
     * @return
     */
    private boolean canClickDateRange(Date startDate, Date endDate, Date date) {
        int i = startDate.compareTo(date);
        int i1 = endDate.compareTo(date);
        return i <= 0 && i1 >= 0;
    }
